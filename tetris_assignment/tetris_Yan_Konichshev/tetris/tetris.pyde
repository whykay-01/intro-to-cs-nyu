...
class Block:
    ...
    # checks whether the movement is OK to implement the movement of a falling block   
    
    def movement(self):
        if self.move and self.col_coordinate + self.move >= 0 and self.col_coordinate + self.move < NUM_COLS:
            self.col_coordinate += self.move
    
    # makes the block to move one block down at a time
...
class Game(list):
   ...
   # checks whether 4 blocks are on top of each other
    ...
    # generates the grid for the game to be displayed
    ...
    # increasing the speed of the game if the score is not incremented
            ...
            # check whether some column is full, so the game does not place any additional blocks to that column

            if self.last_block.row_coordinate == 0:
                self.unfilled_cols.remove(self.last_block.col_coordinate)
            
            # checks for the game over condition (full columns, and as a result: full board)
...
# built-in fucntions which are responsible for the actions that program should take after the pressed left or right keys
# here, it moves the falling block either one block right, or one block left
...
# re-initializing (re-starting) the game in case there is a full board
def mouseClicked():
    global game
    if game.game_over == True:
        game = Game()
